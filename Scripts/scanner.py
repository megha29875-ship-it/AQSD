...
from scoring_v2 import score_option_setup, score_investment_setup
...
def read_market_bias() -> str:
    """Read Market Bias from the Market Pulse worksheet."""
    ...
    try:
        pulse = pd.read_excel(
            OUTPUT_FILE,
            sheet_name="Market Pulse",
            header=None,
        )

        for _, row in pulse.iterrows():
            if str(row.iloc[0]).strip() == "Market Bias":
...
for number, symbol in enumerate(symbols, start=1):
    ...
    try:
        ...
        option_result = score_option_setup(
            ...
            st_gap_percent=st_gap_percent,
            breakout_20d=breakout_20d,
            ema20_cross=fresh_ema20_cross,
            rsi_cross_50=rsi_cross_50,
            adx_rising=adx_rising,
            market_bias=MARKET_BIAS,
        )

        investment_result = score_investment_setup(
            close=close,
            ema50=ema50,
            ema200=ema200,
            high_52w=high_52w,
        ...
        investment_results.append(
            {
                ...
                "RSI14": round(rsi, 2),
                "ADX14": round(adx, 2),
                "Investment Score": investment_result.score,
                "Investment Confidence": investment_result.confidence,
                "Investment Grade": investment_result.grade,
                "Investment Action": investment_result.action,
                "Investment Reasons": " | ".join(investment_result.reasons),
            }
        )
...
try:
    writer_mode = "a" if OUTPUT_FILE.exists() else "w"
